...
To prevent our code to break when there's an error, we'll use the tryCatch() function inside our loop to catch any error and handle it in a particular way. It works like and if-else if-else statement, where you say "If there is no error, do this. If there's an error do this other thing." To show how this works, we'll modify the second weather station in Romania, and then try to run our previous code.
...
```{r}
single_download_noaa_ghcnm <- function(stationId) {
  noaa_url <- "https://www.ncei.noaa.gov/data/ghcnm/v4/precipitation/access/"
  folder_path <- "data/raw/weather_stations/"
  file_path <- paste0(folder_path, stationId, ".csv")
  error_message <- paste0("There was an error trying to download data for stationId: ", stationId, ". Here is the error message: ")
  if (file.exists(file_path)) {
    
  } else {
  tryCatch(
      download.file(url = paste0(noaa_url, stationId, ".csv"), destfile = file_path, quiet = TRUE), error = function(e) {
        print(error_message)
        print(conditionMessage(e))
      }
    )
...
country_download_noaa_ghcnm <- function(data_path, countries) {
  ...
  return(paste0("Number of stations in ", countries, ": ", length(stationid_vector)))
}
```

We can try it with another country, Honduras (HO):
```{r}
